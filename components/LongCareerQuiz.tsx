...
import { suggestCareersLong } from '../services/geminiService';
...
const LongCareerQuiz: React.FC = () => {
  usePageMeta(
    ...
    "Take our 15-question career assessment for a deep analysis of your skills and personality, leading to highly personalized career recommendations."
  );
  ...
  const handleSubmit = async (finalAnswers: Record<string, string>) => {
    ...
    setSuggestions([]);
    try {
      const result = await suggestCareersLong(finalAnswers);
  ...
  const progressPercentage = ((currentQuestionIndex + 1) / LONG_QUIZ_QUESTIONS.length) * 100;

  if (isLoading) {
    return (
      <div className="text-center mt-8 bg-white dark:bg-gray-800 p-12 rounded-xl shadow-lg">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-green-600 mx-auto"></div>
        <p className="mt-4 text-gray-600 dark:text-gray-300 text-lg">Performing deep analysis on your answers...</p>
        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">This will give you highly personalized results!</p>
      </div>
    );
  }
  ...
  if (suggestions.length > 0) {
    return (
      <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg animate-fadeInUp">
        <h2 className="text-3xl font-bold text-center text-gray-800 dark:text-white">Your In-Depth Career Suggestions</h2>
        <div className="mt-8 space-y-6">
          {suggestions.map((suggestion, index) => (
            ...
            >
              ...
              <p className="mt-4 text-sm text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-700 p-3 rounded-md"><strong className="dark:text-gray-200">Why it's a great fit for you:</strong> {suggestion.reasoning}</p>
        ...
        <div className="text-center mt-8">
          <button onClick={handleReset} className="text-green-600 dark:text-green-400 font-semibold hover:underline">Take the assessment again</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white text-center animate-fadeInUp">In-Depth Career Assessment</h1>
        <p className="text-center mt-2 text-gray-600 dark:text-gray-300 animate-fadeInUp animation-delay-100">Answer 15 questions for a detailed analysis.</p>

        <div className="mt-8">
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4 mb-6 relative">
                <div className="bg-green-600 h-4 rounded-full flex items-center justify-center text-xs font-medium text-white" style={{ width: `${progressPercentage}%`, transition: 'width 0.5s ease-in-out' }}>
                   {currentQuestionIndex + 1} / {LONG_QUIZ_QUESTIONS.length}
                </div>
            </div>
          
            <div key={currentQuestionIndex} className="animate-fadeInUp">
                <p className="text-lg text-gray-800 dark:text-gray-200 font-semibold">{currentQuestion.question}</p>
